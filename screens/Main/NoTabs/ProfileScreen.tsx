...
const ProfileScreen = () => {
  ...
  const {currentProfileUrl , logout , userMobileNum , userName , userEmailID} = userStore()
  const accountSettings = [
    
    { id: 2, name: 'Order History', subtitle: 'Check your past purchases', route: 'OrderLists', icon: <Package size={18} /> },
    ...
    { id: 5, name: 'Help & Support', subtitle: '24/7 customer assistance', route: 'HelpSupportScreen', icon: <Headphones size={18} /> },
  ];
  ...
  const renderProfileInfo = () => (
    <View style={{ alignItems: 'center', paddingVertical: 24, backgroundColor: COLORS.background }}>
      <View style={{
        ...
        <Image
          source={{ uri: currentProfileUrl }}
          style={{ width: '100%', height: '100%', borderRadius: 40 }}
        />

        <TouchableOpacity
        onPress={()=>navigation.navigate('ProfileUploadScreen')}
        style = {{position : 'absolute' , right : 3 , bottom : 5 , backgroundColor : '#e6e6e685' , borderRadius : 999 , padding : 3 , alignItems : 'center' , justifyContent : 'center'}}>
      ...
      <Text style={{ fontSize: 12, color: COLORS.textMuted, marginTop: 2, letterSpacing: 0.3 }}>
        {userEmailID}   {userMobileNum}
      </Text>
      
      <TouchableOpacity
       onPress={()=>{
        navigation.navigate('EditUserDetailsScreen')
       }}
       style={{
        marginTop: 16,
        paddingHorizontal: 20,
        ...
        <Text style={{ fontSize: 12, fontWeight: '600', color: COLORS.textMain }}>Edit Bio</Text>
        <Edit size={12} />
      </TouchableOpacity>
    </View>
  );
